...
class BountyExtractor:
    """
    Utility class for extracting bounty information from GitHub issues.
    """
    ...
    def is_bounty_issue(title: str, labels: List[Dict[str, Any]]) -> bool:
        ...
        if BountyExtractor.has_bounty_label(labels):
            return True
            ...
            for pattern in label_patterns:
                ...
                if match:
                    if len(match.groups()) == 3 and match.group(3) in ['gold', 'silver', 'platinum']:
                        ...
                        unit = BountyExtractor.UNIT_MAPPINGS.get(match.group(2).lower(), match.group(2).lower())
                        ...
                        currency = BountyExtractor.CURRENCY_MAPPINGS.get(currency.lower(), currency)
        ...
        for pattern in patterns:
            ...
            for match in matches:
                if len(match.groups()) == 3 and match.group(3) in ['gold', 'silver', 'platinum']:
                    ...
                    currency = BountyExtractor.CURRENCY_MAPPINGS.get(currency.lower(), currency.upper())
    ...
    def extract_bounty_info(
        ...
        amount, currency = BountyExtractor.extract_from_labels(issue.get('labels', []))
        
        # If not found in labels, try title and body
        if not amount or not currency:
            amount, currency = BountyExtractor.extract_from_text(
                issue.get('title', ''), 
                issue.get('body', '')
            )
